...
il nous faut connaître les degrés d'avancement des réactions calculées dans le bilan de matière. Ainsi, nous trouvons la relation suivante:
...
La réaction ayant lieu dans le four n'est que la combustion du méthane:
...
Nous posons: $\Delta H_{Reacteur} = a$ (trouvé à l'aide de la relation \ref(eq:deltaHreacteur)), $\Delta H_{Four} = b$, 
et $DB$ la quantité de \ce{CH_4} nécessaire dans le four.
On trouve alors:

$DB = \unit{\dfrac{a}{b}}{\mol\per\day}$
